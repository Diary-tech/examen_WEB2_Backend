...
CREATE TABLE questions (
    ...
    CONSTRAINT question_points_positive
        CHECK (points > 0),

    CONSTRAINT question_position_positive
        CHECK (position >= 0)
);
...
CREATE TABLE choices (
    ...
    CONSTRAINT choice_position_positive
        CHECK (position >= 0),

    CONSTRAINT unique_choice_position
        UNIQUE (question_id, position),

    CONSTRAINT unique_choice_id_question
...
CREATE TABLE attempts (
    ...
    CONSTRAINT attempt_score_positive
        CHECK (score >= 0),

    CONSTRAINT unique_student_exam_attempt
...
CREATE TABLE answers (
    ...
    CONSTRAINT fk_answer_choice
        ...
        REFERENCES choices(question_id, id)
        ON DELETE RESTRICT,
...
CREATE INDEX idx_exams_start_at
    ON exams(start_at);

CREATE INDEX idx_questions_exam_id
    ON questions(exam_id);

CREATE INDEX idx_choices_question_id
    ON choices(question_id);

CREATE INDEX idx_attempts_exam_id
    ON attempts(exam_id);
